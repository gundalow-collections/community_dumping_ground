...
DOCUMENTATION = '''author: Marcos Diez <marcos (at) unitron.com.br>
lookup: mongodb
short_description: lookup info from MongoDB
description:
- The ``MongoDB`` lookup runs the *find()* command on a given *collection* on a given
  *MongoDB* server.
- The result is a list of jsons, so slightly different from what PyMongo returns.
  In particular, *timestamps* are converted to epoch integers.
options:
  connect_string:
    description:
    - Can be any valid MongoDB connection string, supporting authentication, replicasets,
      etc.
  ...
  sort:
    ...
    default: '[]'
notes:
- Please check https://api.mongodb.org/python/current/api/pymongo/collection.html?highlight=find#pymongo.collection.Collection.find
  for more details.
requirements:
