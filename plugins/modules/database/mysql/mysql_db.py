...
DOCUMENTATION = r'''module: mysql_db
...
options:
  ...
  collation:
    description:
    - Collation mode (sorting). This only applies to new table/databases and does
      not update existing ones, this is a limitation of MySQL.
  encoding:
    description:
  ...
  target:
    ...
    - Location, on the remote host, of the dump file to read from or write to. Uncompressed
      SQL files (C(.sql)) as well as bzip2 (C(.bz2)), gzip (C(.gz)) and xz (Added
      in 2.0) compressed files are supported.
  single_transaction:
    description:
...
- This module is B(not idempotent) when I(state) is C(import), and will import the
  dump file each time if run more than once.
...
from ansible_collections.community.dumping_ground.plugins.module_utils.database import mysql_quote_identifier
