...
DOCUMENTATION = '''module: gitlab_project_variable
...
- When a project variable does exist, its value will be updated when the values are
  different.
- Variables which are untouched in the playbook, but are not untouched in the GitLab
  project, they stay untouched (I(purge) is C(false)) or will be deleted (I(purge)
  is C(true)).
author:
...
options:
  ...
  purge:
    description:
    - When set to true, all variables which are not untouched in the task will be
      deleted.
    default: false
    type: bool
  vars:
