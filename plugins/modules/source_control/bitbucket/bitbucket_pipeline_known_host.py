...
DOCUMENTATION = r'''module: bitbucket_pipeline_known_host
short_description: Manages Bitbucket pipeline known hosts
description:
- Manages Bitbucket pipeline known hosts under the "SSH Keys" menu.
- The host fingerprint will be retrieved automatically, but in case of an error, one
  can use I(key) field to specify it manually.
author:
...
options:
  ...
  state:
    ...
    - present
notes:
- Bitbucket OAuth consumer key and secret can be obtained from Bitbucket profile ->
...
from ansible_collections.community.dumping_ground.plugins.module_utils.source_control.bitbucket import BitbucketHelper
