...
DOCUMENTATION = '''module: zabbix_template
...
- Multiple templates can be created or modified at once if passing JSON or XML to
  module.
author:
- sookido (@sookido)
- Logan Vig (@logan2211)
...
options:
  ...
  template_xml:
    ...
    - You are advised to pass XML structure matching the structure used by your version
      of Zabbix server.
    - Custom XML structure can be imported as long as it is valid, but may not yield
      consistent idempotent results on subsequent runs.
  ...
  template_groups:
    ...
    - Replaces the current host groups the template belongs to if the template is
      already present.
  ...
  link_templates:
    ...
    - Templates that are not specified and are linked to the existing template will
      be only unlinked and not cleared from the template.
    required: false
    type: list
  clear_templates:
    description:
  ...
  macros:
    ...
    - Macros that are not specified and are present on the existing template will
      be replaced.
  ...
  state:
    ...
    - On C(state=present) template will be created/imported or updated depending if
